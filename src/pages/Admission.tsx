...
const StatusBadges = ({ status }: { status: AdmissionType['status'] }) => {
  ...
  if (isCounsellingActive) {
    activeStatuses.push(
      <Badge key="counselling" className="bg-indigo-100 text-indigo-800 border-indigo-200">
        Counselling Active
      </Badge>
    );
  }

  // If no status is active, show closed badge
  if (activeStatuses.length === 0) {
    return <Badge variant="outline">Closed</Badge>;
  }

  return <div className="flex flex-wrap gap-1 justify-end">{activeStatuses}</div>;
};
  ...
  useEffect(() => {
    if (admissions.length > 0) {
      const filtered = admissions.filter(admission => 
        admission.status?.isPublished === true && 
        admission.status?.isApplicationOpen === true
      );
      ...
      <main className="flex-1">
        ...
        <div className="container mx-auto px-4 py-6 md:py-8">
          <div className="max-w-5xl mx-auto space-y-4">
            ...
            {/* Admission Cards - Only show filtered (published + application open) */}
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between mb-2">
                        <h3 className="font-semibold text-foreground text-base md:text-lg flex-1 pr-4">
                          {admission.title}
                        </h3>

                        <StatusBadges status={admission.status} />
                      </div>
